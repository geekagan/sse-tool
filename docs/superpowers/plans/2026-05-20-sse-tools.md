# @geekagan/sse-toolkit Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build and publish `@geekagan/sse-toolkit`, a production-grade npm package for frontend SSE with stream parsing, reconnection, disconnect handling, and resume-from-checkpoint (断点续传).

**Architecture:** Single package, flat TypeScript source, `_internal/` for shared logic not exported. Two public entry points via subpath exports: `@geekagan/sse-toolkit/fetch` (fetch+ReadableStream mode) and `@geekagan/sse-toolkit/eventsource` (EventSource-like API backed by fetch). Both share the same SSE parser, backoff scheduler, and type system.
...
import type { SSEOptions } from '@geekagan/sse-toolkit'
...
```ts
import type { SSEEvent, SSEState, SSECloseReason, RetryOptions } from '@geekagan/sse-toolkit'
```
...
Select: `minor`, package `@geekagan/sse-toolkit`, description: "Initial release: createFetchSSE and createEventSource".
